...
class cs19b017NN(nn.Module):
  def __init__(self, config, C, H, W):
    super(cs19b017NN, self).__init__()
...
def get_model(training_data_loader, n_epochs):
  for X,y in training_data_loader:
    N, C, H, W =  X.shape
    break;
  config = [[C, C+2, (5,5), 1,1]]
  model = cs19b017NN(config, C, H, W).to(device)
  print(model)
...
def get_model_advanced(training_data_loader, n_epochs, lr,config):
  for X,y in training_data_loader:
    N, C, H, W =  X.shape
    break;
  model = cs19b017NN(config, C, H, W).to(device)
